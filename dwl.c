...
commitnotify(struct wl_listener *listener, void *data)
{
  ...
  if (c->isfullscreen)
	...
	if (c->mon && !wlr_box_empty(&box) && (box.width != c->geom.width - 2 * c->bw
		...
		c->isfloating ? resize(c, c->geom, 1, borders) : arrange(c->mon);
...
dwl_ipc_output_set_client_tags(struct wl_client *client, struct wl_resource *resource, uint32_t and_tags, uint32_t xor_tags)
{
	...
	selected_client = focustop(monitor, 0);
	if (!selected_client)
		return;
...
dwl_ipc_output_set_tags(struct wl_client *client, struct wl_resource *resource, uint32_t tagmask, uint32_t toggle_tagset)
{
	...
	focusclient(focustop(monitor, 0), 1);
	arrange(monitor);
	printstatus();
}
...
kblayoutnotify(Keyboard *kb, int update)
{
	...
	if (update) {
		...
		if (kblayout_perclient && kb->layout_idx != old && (c = focustop(selmon, 0)))
		...
		double dx_unaccel, double dy_unaccel)
{
	...
	if (!surface && !seat->drag && (!cursor_image || strcmp(cursor_image, "left_ptr")) && !cursor_hidden)
		wlr_xcursor_manager_set_cursor_image(cursor_mgr, (cursor_image = "left_ptr"), cursor);
